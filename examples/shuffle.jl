...
@kernel function shfl_suffix_sum_kernel(dst, src)
    I = @index(Global, Linear)
    val = src[I]
    lane = I
    ...
    offset = 1
    while offset < 32
        shuffled_val = @shfl(Down, val, offset, 32, 0xffffffff)
        if lane + offset <= 32
            val += shuffled_val
        end
        offset <<= 1
...
src = cu([1 for i in 1:32])
dst = cu([0 for i in 1:32])
shfl_suffix_sum_kernel(CUDABackend())(dst, src; ndrange=32)
dst  # [32, 31, 30, ..., 2, 1]
