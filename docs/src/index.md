...
- **Vectorized Memory Operations**: Hardware-accelerated vector loads and stores with `vload` and `vstore!`
...
Vectorized loads and stores (`vload`, `vstore!`) use LLVM intrinsic functions to generate efficient vector instructions (`ld.global.v4`, `st.global.v4`). 

**Current limitations:**
- Arrays must be contiguous in memory
- Views are not supported
- Future versions may add fallback paths using tuples for non-contiguous access
