...
@testset "Custom struct vload/vstore" begin
    ...
    @testset "Vec3_1 ($(sizeof(Vec3_1)) bytes, scalar fallback)" begin
        ...
        src = to_device([Vec3_1(Float32(i), Float32(i + 10), Float32(i + 20)) for i in 1:16])
        dst = to_device(fill(zero(Vec3_1), 4))
        test_vload_vec3_1(backend)(src, dst; ndrange=1)
        synchronize(backend)
        ...
        dst2 = to_device(fill(zero(Vec3_1), 8))
        test_vstore_vec3_1(backend)(dst2; ndrange=1)
        synchronize(backend)
        result2 = from_device(dst2)
        ...
        v_src = view(src, 3:16)
        dst3 = to_device(fill(zero(Vec3_1), 4))
        test_vload_vec3_1(backend)(v_src, dst3; ndrange=1)
        synchronize(backend)
        result3 = from_device(dst3)
    ...
    @testset "S6 ($(sizeof(S6)) bytes, scalar fallback)" begin
        ...
        src = to_device([S6(Float16(i), Float16(i + 10), Float16(i + 20)) for i in 1:16])
        dst = to_device(fill(zero(S6), 4))
        test_vload_s6(backend)(src, dst; ndrange=1)
        synchronize(backend)
        result = from_device(dst)
        ...
        dst2 = to_device(fill(zero(S6), 8))
        test_vstore_s6(backend)(dst2; ndrange=1)
        synchronize(backend)
        result2 = from_device(dst2)
        ...
        v_src = view(src, 2:16)
        dst3 = to_device(fill(zero(S6), 4))
        test_vload_s6(backend)(v_src, dst3; ndrange=1)
        synchronize(backend)
        result3 = from_device(dst3)
    ...
    @testset "Vec3_2 ($(sizeof(Vec3_2)) bytes, vectorized path)" begin
        ...
        src = to_device([Vec3_2((UInt8(i), Float16(i + 10), UInt8(i + 20)), Float32(i + 30), Float32(i + 40)) for i in 1:16])
        dst = to_device(fill(zero(Vec3_2), 4))
        test_vload_vec3_2(backend)(src, dst; ndrange=1)
        synchronize(backend)
        result = from_device(dst)
        ...
        dst2 = to_device(fill(zero(Vec3_2), 8))
        test_vstore_vec3_2(backend)(dst2; ndrange=1)
        synchronize(backend)
        result2 = from_device(dst2)
        ...
        v_src = view(src, 2:16)
        dst3 = to_device(fill(zero(Vec3_2), 4))
        test_vload_vec3_2(backend)(v_src, dst3; ndrange=1)
        synchronize(backend)
        result3 = from_device(dst3)
    ...
    @testset "TupleStruct ($(sizeof(TupleStruct)) bytes)" begin
        ...
        src = to_device([TupleStruct(Float16(i), UInt8(i + 10)) for i in 1:16])
        dst = to_device(fill(zero(TupleStruct), 4))
        test_vload_ts(backend)(src, dst; ndrange=1)
        synchronize(backend)
        result = from_device(dst)
        ...
        dst2 = to_device(fill(zero(TupleStruct), 8))
        test_vstore_ts(backend)(dst2; ndrange=1)
        synchronize(backend)
        result2 = from_device(dst2)
        ...
        v_src = view(src, 2:16)
        dst3 = to_device(fill(zero(TupleStruct), 4))
        test_vload_ts(backend)(v_src, dst3; ndrange=1)
        synchronize(backend)
        result3 = from_device(dst3)
