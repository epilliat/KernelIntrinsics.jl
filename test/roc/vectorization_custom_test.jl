using Test, AMDGPU, KernelAbstractions, KernelIntrinsics
...
@testset "Custom struct vload/vstore" begin
    ...
    @testset "Vec3_1 ($(sizeof(Vec3_1)) bytes, scalar fallback)" begin
        ...
        src = ROCArray([Vec3_1(Float32(i), Float32(i + 10), Float32(i + 20)) for i in 1:16])
        dst = ROCArray(fill(zero(Vec3_1), 4))
        test_vload_vec3_1(ROCBackend())(src, dst; ndrange=1)
        ...
        dst2 = ROCArray(fill(zero(Vec3_1), 8))
        test_vstore_vec3_1(ROCBackend())(dst2; ndrange=1)
        synchronize(ROCBackend())
        result2 = Array(dst2)
        ...
        # View
        v_src = view(src, 3:16)
        dst3 = ROCArray(fill(zero(Vec3_1), 4))
        test_vload_vec3_1(ROCBackend())(v_src, dst3; ndrange=1)
        synchronize(ROCBackend())
        result3 = Array(dst3)
    ...
    @testset "S6 ($(sizeof(S6)) bytes, scalar fallback)" begin
        ...
        src = ROCArray([S6(Float16(i), Float16(i + 10), Float16(i + 20)) for i in 1:16])
        dst = ROCArray(fill(zero(S6), 4))
        test_vload_s6(ROCBackend())(src, dst; ndrange=1)
        synchronize(ROCBackend())
        result = Array(dst)
        ...
        dst2 = ROCArray(fill(zero(S6), 8))
        test_vstore_s6(ROCBackend())(dst2; ndrange=1)
        synchronize(ROCBackend())
        result2 = Array(dst2)
        ...
        # View
        v_src = view(src, 2:16)
        dst3 = ROCArray(fill(zero(S6), 4))
        test_vload_s6(ROCBackend())(v_src, dst3; ndrange=1)
        synchronize(ROCBackend())
        result3 = Array(dst3)
    ...
    @testset "Vec3_2 ($(sizeof(Vec3_2)) bytes, vectorized path)" begin
        ...
        src = ROCArray([Vec3_2((UInt8(i), Float16(i + 10), UInt8(i + 20)), Float32(i + 30), Float32(i + 40)) for i in 1:16])
        dst = ROCArray(fill(zero(Vec3_2), 4))
        test_vload_vec3_2(ROCBackend())(src, dst; ndrange=1)
        synchronize(ROCBackend())
        result = Array(dst)
        ...
        dst2 = ROCArray(fill(zero(Vec3_2), 8))
        test_vstore_vec3_2(ROCBackend())(dst2; ndrange=1)
        synchronize(ROCBackend())
        result2 = Array(dst2)
        ...
        # View
        v_src = view(src, 2:16)
        dst3 = ROCArray(fill(zero(Vec3_2), 4))
        test_vload_vec3_2(ROCBackend())(v_src, dst3; ndrange=1)
        synchronize(ROCBackend())
        result3 = Array(dst3)
    ...
    @testset "TupleStruct ($(sizeof(TupleStruct)) bytes)" begin
        ...
        src = ROCArray([TupleStruct(Float16(i), UInt8(i + 10)) for i in 1:16])
        dst = ROCArray(fill(zero(TupleStruct), 4))
        test_vload_ts(ROCBackend())(src, dst; ndrange=1)
        synchronize(ROCBackend())
        result = Array(dst)
        ...
        dst2 = ROCArray(fill(zero(TupleStruct), 8))
        test_vstore_ts(ROCBackend())(dst2; ndrange=1)
        synchronize(ROCBackend())
        result2 = Array(dst2)
        ...
        # View
        v_src = view(src, 2:16)
        dst3 = ROCArray(fill(zero(TupleStruct), 4))
        test_vload_ts(ROCBackend())(v_src, dst3; ndrange=1)
        synchronize(ROCBackend())
        result3 = Array(dst3)
